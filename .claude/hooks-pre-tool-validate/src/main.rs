...
fn is_protected_config(file_path: &str) -> bool {
    // パスを正規化（Windows \ → /、小文字化で case-insensitive 比較）
    ...
    PROTECTED_CONFIG_FILES.iter().any(|&protected| {
        ...
        if protected == ".husky" {
            // ディレクトリ名 → パス内のどこかにディレクトリとして含まれるかチェック
...
mod tests {
    ...
    #[test]
    fn protects_husky_subpath() {
        assert!(is_protected_config(".husky/pre-commit"));
    }

    #[test]
    fn protects_husky_subpath_windows() {
        assert!(is_protected_config(r"e:\work\project\.husky\pre-commit"));
    }

    #[test]
    fn protects_husky_dir_itself() {
        assert!(is_protected_config(".husky"));
    }

    #[test]
    fn protects_husky_case_insensitive() {
        assert!(is_protected_config(".HUSKY/pre-commit"));
        assert!(is_protected_config(r"e:\work\.Husky\pre-commit"));
    }
}
