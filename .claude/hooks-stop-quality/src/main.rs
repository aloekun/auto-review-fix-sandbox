...
fn main() {
    // stdin を消費（読み取り失敗時は品質ゲートをバイパスさせない）
    let mut input = String::new();
    if let Err(e) = io::stdin().read_to_string(&mut input) {
        let decision = BlockDecision {
            decision: "block".to_string(),
            reason: format!("品質ゲート: stdinの読み取りに失敗しました: {}", e),
        };
    ...
    let hook_input: HookInput = match serde_json::from_str(&input) {
        Ok(v) => v,
        Err(e) => {
            // パース失敗 → 品質ゲートをバイパスさせない
            let reason = format!(
                "品質ゲート: 入力JSONのパースに失敗しました: {}",
                e
            );
            let decision = BlockDecision {
                decision: "block".to_string(),
...
mod tests {
    ...
    fn quality_steps_names() {
        ...
        assert_eq!(names, vec!["py-lint", "py-test", "py-test:e2e"]);
    }
